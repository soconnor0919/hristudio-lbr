...
\subsection{The organization of a user study}

With HRIStudio, we define a hierarchical organization of elements to express WoZ user studies for HRI research. An experimenter starts by creating and configuring a user study a \emph{study} element. The \emph{study} will comprise multiple instantiations of one same experimental script encapsulated in an element called \emph{experiment}, which captures the experiences of a specific human subject with the robot designated in the script. Each \emph{experiment} comprises a sequence of \emph{step} elements and each of these comprises a sequence of \emph{action} elements which are executed asynchronously as commanded by the wizard in charge.
The system executes the \emph{experiment} script asynchronously allowing the wizard to respond in real time to the reactions of their human subject simulating the robot's autonomous intelligence. In order to enforce consistency across multiple runs of the \emph{experiment}, HRIStudio can constrain the interventions that a wizard can interject spontaneously according to specifications encoded in the \emph{study} element. At the lowest level, each \emph{action} models with fine granularity a single interaction between robot and human-- it may correspond to a specific physical or virtual task for the robot or the wizard to execute including but not limited to robot movements, verbal interactions, or awaiting for a human subject's input. 

Figure~\ref{experimentalarchitecture} illustrates this hierarchy of elements with a practical example. By organizing a user study's experimental procedures into this hierarchical structure, researchers can design complex HRI studies while guiding the wizard to follow a consistent set of instructions.
